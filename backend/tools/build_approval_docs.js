...
const overblik = makeDoc([
  ...
  P(`Kontrollerne er delt i momsrelevans-moduler: en momskerne (default TIL) med de kontroller, der har en direkte momsfaglig konsekvens, og udvidede moduler (forensic/statistik, e-handel/særordninger, datakvalitet, dublet-recovery) der er default FRA og kan tændes efter behov. En standardkørsel viser dermed momskernen — den egentlige momsgennemgang — frem for en mur af flag.`),
  ...
  table(["Område", "Status", "Bemærkning"], [
    ...
    ["Auth & adgang", "Grøn", "Central BALAI-brugerstyring (login/setup/admin på auth.balai.dk); appen håndhæver adgang til slug “vat”."],
    ["HTTP-sikkerhed", "Grøn", "Stram CSP uden CDN'er/unsafe-*, HSTS, fuld header-pakke; UI er self-hostet."],
    ["Input Excel/CSV + SAF-T", "Grøn", "Fladt udtræk eller dansk SAF-T Financial (v1.0/2.0/2.1); best-effort, XML-hærdet."],
    ["Momsrelevans-moduler", "Grøn", `Momskerne default TIL; udvidede moduler default FRA. Hver kontrol bærer analyse_modul + default_aktiv i kataloget.`],
    ...
    ["Stress-test på rigtige data", "Gul", "To reelle SAF-T-klientfiler kørt (robust scope bekræftet); fuld rød→reel-optælling udestår."],
  ], [2700, 1100, 5560]),
  ...
  P(`Status pr. ${DATE}: Alt der kan lukkes i kode og dokumentation internt, er på plads — versioneret regelkatalog, sporbarhedsmatrix, uafhængig valideringssuite (${N_ACTIVE}/${N_ACTIVE} gated i CI), central BALAI-brugerstyring, stram HTTP-sikkerhed, dobbelt input (Excel/CSV + SAF-T), momsrelevans-moduler og drift i EU. De resterende punkter er bevidst markeret som åbne, fordi de kræver EY-beslutninger, faglig pinning eller eksterne parter.`, { bold: true }),
]);
...
const arch = makeDoc([
  ...
  H2("3.1 I scope"),
  ...bullets([
    `${N_TOTAL} momskontroller i 12 kategorier (heraf ${N_ACTIVE} aktive på et fladt bogføringsudtræk), delt i momsrelevans-moduler (momskerne default TIL; udvidede moduler default FRA).`,
    "Import af momsudtræk fra Excel/CSV med fleksibel, alias-baseret kolonnegenkendelse (dansk + engelsk + ERP-navne) ELLER fra dansk SAF-T Financial (v1.0/2.0/2.1).",
  ...
  table(["Lag", "Teknologi"], [
    ["Web-lag", "Python 3.13 + FastAPI/uvicorn (ruter, jobstyring, auth-wiring, sikkerhedsheaders)"],
    ["Kontrol-logik", "backend/analytics/: engine + 12 kategorimoduler + delte momsregler + analyse-moduler (ren Python)"],
    ["Import", "backend/parsers/: excel_parser + data_adapter (fladt Excel/CSV) og saft_parser + upload_router (SAF-T)"],
    ["Auth/governance", "Central BALAI-brugerstyring (central_auth.py; delt login på .balai.dk, fælles Postgres) + lokal revisionslog (SQLite)"],
  ...
  H2("4.2 Komponenter"),
  ...bullets([
    "backend/analytics/engine.py — orkestrerer alle kontroller, filtrerer til aktive analyse-moduler og bygger den klassificerede rapport (scores, impact, severity).",
    "backend/analytics/categories/cat01–cat12 — de 12 kategorimoduler med kontrollerne.",
    "backend/analytics/modules.py — analyse-modul-registeret (kontrol → modul; momskerne default TIL, øvrige FRA; styres af ANALYTICS_MODULES).",
    "backend/analytics/vat_rules.py — delte danske momsregler/helpers (satser, EU-lande, momsnr-format, CVR mod-11, statistik) + momsrelevans-scope (is_non_vat_account).",
    "backend/analytics/standard_accounts.py — StandardAccountID → nature (balance ≥ 5000 / resultat 1000–4999) fra standardkontoplanen; robust scope-signal.",
    "backend/parsers/ — excel_parser.py + data_adapter.py (fladt udtræk) og saft_parser.py + upload_router.py (SAF-T, best-effort, XML-hærdet).",
    "backend/catalog/ — versioneret rules.json (+ analyse_modul/default_aktiv) + rule_notes.json (kilde/test/beslutninger).",
  ...
  ...numbered([
    "Brugeren logger ind og uploader et momsudtræk (Excel/CSV) eller en SAF-T-fil (.xml). Små filer analyseres synkront; store filer (≥50 MB, op til 2 GB) kører som et asynkront baggrundsjob med statuspolling.",
    "Upload-routeren vælger parser på filendelse/indholds-sniff: Excel/CSV genkendes alias-baseret og tilpasses; SAF-T mappes best-effort (namespace-agnostisk, XML-hærdet) — begge til samme kanoniske transaktions-/linjestruktur.",
  ...
  P("Ved SAF-T-input leveres en rigere struktur direkte fra kilden: kontoplan (AccountType + StandardAccountID), momstabel, kunde-/leverandørstamdata (CVR/land) og fradragsbegrænsning (NonDeductibleAmount). Parseren bærer bl.a. standard_account_id med på linjen, hvilket driver det robuste momsrelevans-scope. Parsningen er best-effort: den kører også på en ugyldig eller fejlmærket SAF-T (fx en fil der deklarerer v1.0, men indeholder et fuldt datasæt)."),

  H1("10. Sikkerhed og databehandling (resumé)"),
  P("Adgang kræver login via den centrale BALAI-brugerstyring (login/setup/invitationer/admin på auth.balai.dk). Appen læser den delte, signerede session-cookie (delt på .balai.dk med samme SECRET_KEY), validerer den mod den fælles Postgres og håndhæver adgang til værktøjets slug «vat»; uautoriserede API-kald får 401, web-ruter omdirigeres til central login. Tilstandsændrende handlinger er CSRF-beskyttede. Alle svar bærer en stram CSP uden CDN'er samt HSTS og øvrige sikkerhedsheaders. Inputfilen slettes straks efter kørslen, resultater persisteres ikke, og en append-only revisionslog registrerer kun metadata. Detaljer i det separate dokument Sikkerhed og databehandling."),
  ...
  ...bullets([
    ...
    "Materialitetstærskler og severity-vægte er samlet i analytics/materiality.py og kan engagement-kalibreres via MATERIALITY_*-miljøvariabler (defaults = hidtidig adfærd).",
  ]),
  ...
  table(["Modul", "Ansvar"], [
    ["main.py", "Tyndt FastAPI web-lag: ruter, upload/jobstyring, auth-wiring (central_auth), sikkerhedsheaders."],
    ["central_auth.py", "Central BALAI-brugerstyring: validerer delt session-cookie mod fælles Postgres, håndhæver adgang til slug «vat» (require_tool)."],
    ["auth.py", "Lokal rest: CSRF-verifikation, templates og revisionslog-opsætning."],
    ["audit_log.py", "Append-only revisionslog (kun metadata)."],
    ["analytics/engine.py", "Orkestrering + modul-filtrering + rapportbygning (scores, impact, severity)."],
    ["analytics/categories/*", "De 12 kategorimoduler med kontrollerne."],
    ["analytics/modules.py", "Analyse-modul-register (momskerne default TIL; øvrige FRA)."],
    ["analytics/vat_rules.py", "Delte danske momsregler/helpers + momsrelevans-scope."],
    ["analytics/standard_accounts.py", "StandardAccountID → balance/resultat (standardkontoplan)."],
    ["parsers/*", "Excel/CSV (excel_parser + data_adapter) og SAF-T (saft_parser + upload_router)."],
...
const sec = makeDoc([
  ...
  table(["Kategori", "Indhold", "Opbevaring"], [
    ["Kundedata (momsudtræk)", "Den uploadede Excel/CSV eller SAF-T-fil med transaktioner — kan indeholde modparts-navne, momsnumre m.v.", "Slettes straks efter kørslen."],
    ["Analyseresultat", "Fund + sporbarhed til kildetransaktioner, genereret for kørslen.", "Kun i hukommelsen; persisteres ikke til disk."],
    ["Bruger-/adgangsdata", "Brugerkonti, roller og adgange håndteres af den centrale BALAI-brugerstyring (auth.balai.dk). Værktøjet gemmer ikke selv passwords.", "Fælles Postgres (central auth) — ikke i dette værktøj."],
    ["Revisionslog", "Kun metadata: hændelse, aktør (email), tidspunkt, IP, filstørrelse, udfald.", "Lokal SQLite, append-only, EU-volumen."],
  ], [2400, 4800, 2160]),
  ...
  ...bullets([
    ...
    "De eneste vedvarende persondata i selve værktøjet er revisionslog-metadata (aktørens email m.v.) til governance. Brugerkonti ligger i den centrale BALAI-brugerstyring (fælles Postgres), ikke i værktøjet.",
    "Værktøjet gemmer ikke passwords; credentials håndteres udelukkende af den centrale brugerstyring. Revisionsloggen indeholder aldrig momsdata/kundedata — kun metadata.",
    "Transport sker over HTTPS. Databehandleraftale og endelig dataplacering fastlægges af driftsplatformen (EY).",
  ]),

  H1("4. Adgang"),
  ...bullets([
    "Adgang kræver login via den centrale BALAI-brugerstyring. Login, opret-administrator (/setup), invitationer og brugeradministration sker KUN centralt på auth.balai.dk — ikke i dette værktøj.",
    "Værktøjet læser den delte, signerede session-cookie (sat på .balai.dk med samme SECRET_KEY), validerer den mod den fælles Postgres og håndhæver adgang til værktøjets slug «vat» (require_tool).",
    "Der findes ingen credentials i kode eller miljøvariabler; værktøjet gemmer ingen passwords. Den tidligere hardcodede HTTP Basic Auth er fjernet.",
    "Alle tilstandsændrende handlinger er CSRF-beskyttede (konstant-tid-sammenligning; header eller form-felt).",
    "Analyse-API'erne kræver login; uautoriserede kald får 401 (API) eller omdirigeres til den centrale login (web).",
  ]),
  ...
  ...bullets([
    ...
    "Brute force mod login: håndteres af den centrale BALAI-brugerstyring (login/rate-limit ligger på auth.balai.dk); værktøjet validerer kun den signerede session mod den fælles database.",
    "Forfalskning af session: cookien er signeret (itsdangerous, HMAC) med en SECRET_KEY der deles med central auth; token_version håndhæves, så blokering/revoke slår igennem øjeblikkeligt. Værktøjet har ingen lokal password-database at kompromittere.",
  ]),

  H1("7. Revisionslog"),
  P("En append-only revisionslog (egen SQLite-database) registrerer værktøjets analysekørsler som metadata (aktørens email, tidspunkt, IP, filstørrelse, udfald). Login-, administrator- og invitationshændelser logges centralt i BALAI-brugerstyringen. Loggen giver et governance-egnet «hvem gjorde hvad, hvornår»-spor og indeholder aldrig momsnumre, beløb, navne eller andet kundedata."),
...
const hosting = makeDoc([
  ...
  ...numbered([
    ...
    "Sæt SECRET_KEY IDENTISK med den centrale BALAI-auth (binder den delte session-cookie på .balai.dk). Sæt DATABASE_URL til den fælles Postgres og evt. AUTH_BASE_URL (default https://auth.balai.dk).",
    "Sæt AUDIT_DB_PATH=/data/audit.db (den lokale revisionslog). Sæt IKKE SESSION_COOKIE_SECURE=0 i produktion.",
    "Deploy via Dockerfile; startCommand (uvicorn --workers 1, $PORT) er pinnet i railway.json.",
    "Opret bruger/adgang til slug «vat» i den centrale brugerstyring (auth.balai.dk) — ikke i værktøjet.",
    "Verificér /health, at login via central auth virker og en genindlæsning bevarer sessionen, sikkerhedsheaders og CI-grøn.",
    "Etabler backup af /data-volumenet (revisionslog) og en dokumenteret sletteproces; brugerdata backes op via den centrale Postgres.",
  ], "n3"),
  P("KRITISK: SECRET_KEY SKAL være identisk med den centrale auth, og DATABASE_URL skal pege på den fælles Postgres — ellers kan den delte session-cookie ikke valideres, og login fejler (redirect-løkke). Den lokale revisionslog (AUDIT_DB_PATH) ligger på det flygtige filsystem uden et monteret /data-volumen og nulstilles ved hver redeploy.", { bold: true }),
  ...
  table(["Variabel", "Formål", "Default"], [
    ["SECRET_KEY", "Signerer/validerer den delte session-cookie. SKAL være IDENTISK med central auth.", "(tilfældig — kun dev)"],
    ["DATABASE_URL", "Fælles Postgres til den centrale brugerstyring (bruger/adgang/token_version).", "(påkrævet i prod)"],
    ["AUTH_BASE_URL", "Central login/redirect-base.", "https://auth.balai.dk"],
    ["AUDIT_DB_PATH", "Sti til den lokale revisionslog (SQLite). SKAL pege på det persistente volumen.", "data/audit.db"],
    ["SESSION_COOKIE_SECURE", "Secure-flag på session-cookie. Lad være usat (=secure) i prod.", "1 (secure)"],
    ["ANALYTICS_MODULES", "Aktive analyse-moduler (komma-liste; “alle” = alt).", "(kun momskernen)"],
    ...
    ["JOB_RETENTION_SECONDS / MAX_JOBS", "TTL / maks-antal for async-jobs i hukommelsen.", "3600 / 100"],
  ...
  ...bullets([
    "Brugerdata (konti/adgange) ligger i den centrale BALAI-Postgres og backes op dér, uden for dette værktøj.",
    "Det persistente /data-volumen (den lokale revisionslog) bør indgå i platformens backup-rutine.",
    "Da kundedata ikke opbevares (input slettes pr. kørsel, resultater persisteres ikke), er gendannelsesomfanget begrænset til revisionsloggen.",
  ]),
