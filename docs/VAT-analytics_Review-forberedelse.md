...
- **Hold Node-artefakter ude af git** (`package.json`, `package-lock.json`, `node_modules/`
  ...
  fejldetektere en Node-app og bygge UDEN Python-deps (`command not found` i deploy-loggen,
  mens en gammel replica holder sitet oppe). Dette var den eneste reelle årsag på Data Extract.
  (En `startCommand`-override i `railway.json` er derimod fin — VIES kører sådan.)
